...
// Profile and Feedback Section JavaScript - Safe Implementation
document.addEventListener("DOMContentLoaded", function() {
    // ======================
    // STAR RATING SYSTEM
    // ======================
    
    // Initialize star rating for desktop
    ...
    // Initialize star rating for mobile
    ...
    // ======================
    // PROFILE FORM & OTP SYSTEM
    // ======================
    
    // Initialize profile data (in a real app, this would come from backend)
    const profileData = {
        name: 'Anand',
    ...
    // Only initialize profile if elements exist
    const profileNameEl = document.getElementById('profileName');
    const mobileProfileNameEl = document.getElementById('mobileProfileName');
    
    if (profileNameEl || mobileProfileNameEl) {
        // Format join date
        const joinDateStr = profileData.joinDate.toLocaleDateString('en-US', { 
            year: 'numeric', 
            month: 'long' 
        });
        ...
        if (profileNameEl) {
            ...
            document.getElementById('joinDate').textContent = `Member since ${joinDateStr}`;
        }
        if (mobileProfileNameEl) {
            mobileProfileNameEl.textContent = profileData.name;
            document.getElementById('mobileJoinDate').textContent = `Member since ${joinDateStr}`;
        }
    ...
    // Setup OTP inputs for both desktop and mobile if they exist
    ...
    function handleOTPSubmit(form, isMobile) {
        ...
        // Reset forms and hide OTP container
        setTimeout(() => {
            if (isMobile) {
                document.getElementById('mobileOtpContainer').classList.add('hidden');
                document.getElementById('mobileProfileFormContainer').style.display = 'block';
...
// Replace the feedback card with the animated version
document.addEventListener("DOMContentLoaded", function() {
    // Create the animated feedback card for desktop
    ...
    // Create the animated feedback card for mobile
    ...
    // Add the CSS for the animated feedback cards
    const style = document.createElement('style');
    style.textContent = `
        /* Desktop Feedback Animation */
        .feedback-box {
            position: relative;
            width: 100%;
            height: 25rem;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function handleFormSubmit(form, isMobile) {
        form.addEventListener('submit', function(e) {
            ...
            const bookingData = {
                ...
                date: formData.get('testRideDate'),
                time: formData.get('testRideTime'),
            ...
            console.log('Test ride booking:', bookingData);
            
            showNotification(
                'success', 
                'Booking Confirmed!', 
                `Thank you for booking a test ride for ${selectedBike.name}. Our team will contact you shortly to confirm the appointment and provide additional details.`
            );
    ...
    if (desktopForm) {
        handleFormSubmit(desktopForm, false);
    }
    if (mobileForm) {
        handleFormSubmit(mobileForm, true);
    }
});

// Fixed JavaScript - Completely separated Quotation and Online Booking systems

document.addEventListener("DOMContentLoaded", function () {
    // =====================================================
    // QUOTATION SYSTEM - Completely isolated
    // =====================================================
    ...
    const QuotationSystem = {
        ...
        updateSummary(isMobile = false) {
            ...
            // Show order summary when all selections are made
        ...
        setupFormSubmission() {
            ...
            const handleSubmit = (isMobile = false) => {
                ...
                showNotification(
                    ...
                    'Thank you for your interest! Our sales team will contact you shortly with detailed pricing information.'
                );
            ...
            if (mobileSubmitBtn) {
                mobileSubmitBtn.addEventListener('click', () => handleSubmit(true));
            }
        },
    ...
    // =====================================================
    // BOOKING SYSTEM - Completely isolated
    // =====================================================

    const BookingSystem = {
        // Bike configuration data
        bikeConfig: {
            access125: {
                name: "Suzuki Access 125",
                variants: {
